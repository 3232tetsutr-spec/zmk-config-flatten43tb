# zmk-config-flatten43tb<!-- omit in toc -->

- Flatten43tb用のZMKファームウェア

## Flatten43tbとは<!-- omit in toc -->
...
![flatten43tb.svg](keymap-drawer/flatten43tb.svg)
...
- 以下のリポジトリをフォークする
  - https://github.com/keebkuro/zmk-config-flatten43tb
...
- `XIAO-SENSE`に`flatten43tb_R-seeeduino_xiao_ble-zmk.uf2`ファイルをドラッグアンドドロップしてください
...
- 右側キーボードと同じ手順で実施可能です
- 書き込むファームウェアは左側キーボード用のものに読み替えてください（`flatten43tb_L-seeeduino_xiao_ble-zmk.uf2`）
...
- キーボードを選択するモーダルが表示されるので、`flatten43tb (cu.usbmodem11301)`を選択してから`接続`をクリック
...
- トラックボールのマウス感度（CPI: Counts Per Inch）を変更したいときは以下のファイルを修正すること
  - [boards/shields/flatten43tb/flatten43tb_R.overlay](boards/shields/flatten43tb/flatten43tb_R.overlay)
- `cpi = <400>;`の値を変更する
  - 値を大きくするほど感度が上がります
